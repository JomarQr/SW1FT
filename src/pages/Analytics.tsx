  ...
  return (
    <div style={{ background: '#111115', border: '1px solid #1E1E22', ...style }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #1E1E22' }}>
        <span style={{ fontFamily: 'DM Sans', fontSize: '13px', fontWeight: 500, color: COLORS.primary }}>{title}</span>
        {subtitle && <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted, marginLeft: '10px' }}>{subtitle}</span>}
      </div>
        ...
        {EU_COUNTRY_ALERTS.map(c => (
          <div key={c.code} style={{ background: '#0F0F12', border: `1px solid ${alertColor(c.alerts)}44`, padding: '10px 8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '12px', fontWeight: 600, color: COLORS.primary }}>{c.code}</span>
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '11px', color: alertColor(c.alerts), fontWeight: 600 }}>{c.alerts}</span>
            </div>
            <div style={{ height: '3px', background: '#1E1E22' }}>
              <div style={{ height: '100%', width: `${(c.alerts / max) * 100}%`, background: alertColor(c.alerts) }} />
            </div>
            <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: COLORS.muted, marginTop: '3px' }}>{c.name}</div>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '8px 0', borderTop: '1px solid #1E1E22' }}>
        <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted }}>Density:</span>
        {[
  ...
  return (
    <div style={{ padding: '24px', minHeight: '100vh', background: '#0A0A0B' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontFamily: 'DM Sans', fontSize: '20px', fontWeight: 600, color: COLORS.primary, margin: '0 0 4px' }}>Analytics</h1>
          <p style={{ fontFamily: 'IBM Plex Mono', fontSize: '11px', color: COLORS.muted, margin: 0 }}>6-week performance report · EU-PSP deployment</p>
        </div>
        <div style={{ display: 'flex', border: '1px solid #1E1E22', overflow: 'hidden' }}>
          {(['weekly', 'monthly'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              background: period === p ? 'rgba(0,229,204,0.1)' : '#111115',
              border: 'none', color: period === p ? COLORS.accent : COLORS.muted,
              fontFamily: 'IBM Plex Mono', fontSize: '10px', padding: '6px 14px', cursor: 'pointer',
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: '#111115', border: '1px solid #1E1E22', padding: '16px' }}>
            <div style={{ fontFamily: 'DM Sans', fontSize: '11px', color: COLORS.muted, marginBottom: '8px' }}>{label}</div>
            <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '22px', fontWeight: 600, color, lineHeight: 1 }}>{value}</div>
            <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted, marginTop: '5px' }}>{sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
        <Card title="Alert Accuracy & False Positive Rate Trend" style={{ flex: 1 }}>
          <div style={{ height: '240px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}>
                ...
                <XAxis dataKey="week" tick={{ fill: COLORS.muted, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: COLORS.muted, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} domain={[0, 100]} />
          ...
          <div style={{ display: 'flex', gap: '20px', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #1E1E22' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '20px', height: '2px', background: COLORS.safe }} /><span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted }}>Alert Accuracy</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '20px', height: '2px', background: COLORS.danger }} /><span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted }}>False Positive Rate</span></div>
          </div>
        </Card>
          ...
          <div style={{ height: '240px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1E1E22" vertical={false} />
                <XAxis dataKey="week" tick={{ fill: COLORS.muted, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: COLORS.muted, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `€${(v / 1000).toFixed(0)}K`} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="baselineLoss" name="Without SentinelLayer" fill="#1E1E22" radius={0} />
                <Bar dataKey="lossPrevented" name="With SentinelLayer" fill={COLORS.accent} radius={0} fillOpacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div style={{ display: 'flex', gap: '20px', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #1E1E22' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: '#1E1E22', border: '1px solid #2A2A32' }} /><span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted }}>Without system</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: COLORS.accent, opacity: 0.8 }} /><span style={{ fontFamily: 'IBM Plex Mono', fontSize: '10px', color: COLORS.muted }}>Prevented</span></div>
          </div>
        </Card>
      </div>
          ...
          <div style={{ height: '260px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={SIGNAL_EFFECTIVENESS} layout="vertical" margin={{ top: 5, right: 10, bottom: 0, left: 130 }}>
                ...
                <XAxis type="number" tick={{ fill: COLORS.muted, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} domain={[0, 100]} />
                <YAxis dataKey="signal" type="category" tick={{ fill: COLORS.primary, fontFamily: 'IBM Plex Mono', fontSize: 10 }} axisLine={false} tickLine={false} width={125} />
                <Tooltip content={<ChartTooltip />} />
              ...
              {SCAM_BREAKDOWN.map(s => (
                <div key={s.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '11px', color: COLORS.primary }}>{s.name}</span>
                  </div>
                  <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '11px', fontWeight: 600, color: s.color }}>{s.value}%</span>
                </div>
              ))}
            </div>
          </div>
