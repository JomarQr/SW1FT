...
function EUHeatmap() {
  ...
  function alertColor(alerts: number): string {
    ...
    if (ratio > 0.75) return 'var(--red)';
    if (ratio > 0.5) return 'var(--orange)';
    if (ratio > 0.25) return 'var(--yellow)';
    return '#3A4A38';
  }
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '16px' }}>
        {EU_COUNTRY_ALERTS.map(c => (
          <div key={c.code} style={{ background: 'var(--surface)', border: `1px solid ${alertColor(c.alerts)}44`, padding: '10px 8px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '8px 0', borderTop: '1px solid var(--bdr)' }}>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: COLORS.muted }}>Density:</span>
        {[
          { label: 'High (>35)', color: 'var(--red)' },
          { label: 'Med (15–35)', color: 'var(--orange)' },
          { label: 'Low (5–15)', color: 'var(--yellow)' },
    ...
    <div style={{ padding: '24px', minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div style={{ display: 'flex', border: '1px solid var(--bdr)', overflow: 'hidden' }}>
          {(['weekly', 'monthly'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              background: period === p ? 'rgba(170,85,227,0.1)' : 'var(--card)',
              ...
              borderRight: p === 'weekly' ? '1px solid var(--bdr)' : 'none',
            }}>
              {p}
            </button>
          ))}
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: 'var(--card)', border: '1px solid var(--bdr)', padding: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
        <Card title="Alert Accuracy & False Positive Rate Trend" style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '20px', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid var(--bdr)' }}>
          ...
          <div style={{ height: '240px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: 10 }}>
                ...
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="baselineLoss" name="Without SW1FT" fill="var(--bdr)" radius={0} />
                <Bar dataKey="lossPrevented" name="With SW1FT" fill={COLORS.accent} radius={0} fillOpacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div style={{ display: 'flex', gap: '20px', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid var(--bdr)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: 'var(--bdr)', border: '1px solid var(--bdr2)' }} /><span style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: COLORS.muted }}>Without system</span></div>
