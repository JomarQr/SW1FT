...
const STATUS_CFG: Record<SessionStatus, { color: string; bg: string }> = {
  SAFE:    { color: '#00CC7A', bg: 'rgba(0,204,122,0.1)'  },
  WATCH:   { color: '#FFB800', bg: 'rgba(255,184,0,0.1)'  },
  ALERT:   { color: '#FF8C00', bg: 'rgba(255,140,0,0.1)'  },
  BLOCKED: { color: '#FF3B5C', bg: 'rgba(255,59,92,0.12)' },
};
...
function KpiCard({
  ...
  return (
    <div style={{
      ...
      borderTop: `2px solid ${topColor ?? '#1A1A1F'}`,
      padding: '12px 14px', flex: 1, minWidth: 0,
    }}>
      <div style={{
        fontFamily: 'JetBrains Mono', fontSize: '8px', fontWeight: 600,
        color: '#3A3A4E', letterSpacing: '0.16em', textTransform: 'uppercase',
        marginBottom: '6px',
      }}>
        {label}
      </div>
      <div style={{
        ...
        color: '#E0E0E8', lineHeight: 1, fontVariantNumeric: 'tabular-nums',
      }}>
        {format(displayed)}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '5px' }}>
        {sub && (
          <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: subColor ?? '#3A3A4E' }}>
            {sub}
          </span>
        )}
        {delta && (
          <span style={{ display: 'flex', alignItems: 'center', gap: '2px', fontFamily: 'JetBrains Mono', fontSize: '9px', color: delta.up === null ? '#3A3A4E' : delta.up ? COLORS.safe : COLORS.danger }}>
...
const ChartTip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: '#0C0C0F', border: '1px solid #1A1A1F', padding: '7px 11px', fontFamily: 'JetBrains Mono', fontSize: '10px' }}>
      <div style={{ color: '#3A3A4E', marginBottom: '3px' }}>{label}</div>
      {payload.map((p: any) => (
        <div key={p.name} style={{ color: p.stroke ?? p.color, display: 'flex', gap: '10px', justifyContent: 'space-between' }}>
          <span style={{ color: '#5A5A6E' }}>{p.name}</span>
...
function SectionHeader({ label, right }: { label: string; right?: React.ReactNode }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', fontWeight: 600, color: '#3A3A4E', letterSpacing: '0.14em', textTransform: 'uppercase' }}>
        {label}
      </span>
      {right}
    </div>
...
export default function Dashboard() {
  ...
  return (
    <div style={{ padding: '16px', minHeight: '100%', background: '#0A0A0B' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '8px', marginBottom: '8px' }}>
        ...
        {/* Live session feed */}
        <div style={{ background: '#0F0F12', border: '1px solid #1A1A1F', overflow: 'hidden' }}>
          <SectionHeader
            ...
            right={
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: '#3A3A4E' }}>refresh 4s · {liveSessions.length} sessions</span>
              </div>
            }
          />
                ...
                <tr style={{ background: '#0B0B0E' }}>
                  {['Session ID', 'User', 'Channel', 'Risk', '', 'Status', 'Amount', ''].map((h, i) => (
                    <th key={i} style={{
                      ...
                      letterSpacing: '0.14em', textTransform: 'uppercase', color: '#2E2E3E',
                      borderBottom: '1px solid #1A1A1F', textAlign: i >= 6 ? 'right' : 'left', whiteSpace: 'nowrap',
                    }}>
                      {h}
                    </th>
                  ))}
                ...
                {liveSessions.map(s => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ cursor: 'pointer', borderBottom: '1px solid #131316' }}
                    ...
                    >
                      ...
                      <td style={{ padding: '6px 10px', fontFamily: 'JetBrains Mono', fontSize: '10px', color: '#4A4A5E', whiteSpace: 'nowrap' }}>{s.userId}</td>
                      <td style={{ padding: '6px 10px', fontFamily: 'JetBrains Mono', fontSize: '9px', color: '#3A3A4E', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{s.channel}</td>
                      <td style={{ padding: '6px 4px 6px 10px' }}>
                        <div style={{ width: '36px', height: '2px', background: '#1A1A1F' }}>
                      ...
                      <td style={{ padding: '6px 10px', fontFamily: 'JetBrains Mono', fontSize: '11px', color: '#8888A0', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                        €{s.transactionAmount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
                      <td style={{ padding: '6px 10px' }}>
                        <ExternalLink size={10} color="#2A2A3A" />
                      </td>
                    </tr>
                  );
                })}
        ...
        <div style={{ background: '#0F0F12', border: '1px solid #1A1A1F', overflow: 'hidden' }}>
          <SectionHeader
            ...
            right={
              <span
                ...
                style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: '#3A3A4E', cursor: 'pointer', letterSpacing: '0.06em' }}
          ...
          {/* Alert table header */}
          <div style={{ display: 'grid', gridTemplateColumns: '32px 1fr auto auto', gap: '0', padding: '5px 12px', borderBottom: '1px solid #131316', background: '#0B0B0E' }}>
            {['RISK', 'SIGNAL', 'AMT', 'AGO'].map(h => (
              <div key={h} style={{ fontFamily: 'JetBrains Mono', fontSize: '8px', fontWeight: 600, color: '#2E2E3E', letterSpacing: '0.12em' }}>{h}</div>
            ))}
          </div>
            ...
            {topAlerts.map(a => (
              <div
                ...
                style={{
                  ...
                  padding: '7px 12px', borderBottom: '1px solid #131316',
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: '#9090A0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '1px' }}>
                    {a.primarySignal}
                  </div>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '8px', color: '#36364A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {a.scamType} · {a.userId}
                  </div>
                </div>
                <div style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: '#5A5A6E', textAlign: 'right', paddingLeft: '8px', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                  {fmtEur(a.transactionAmount)}
                </div>
                <div style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: '#36364A', textAlign: 'right', paddingLeft: '8px', whiteSpace: 'nowrap' }}>
                  {fmtTime(a.time)}
                </div>
              </div>
            ))}
      ...
      <div style={{ background: '#0F0F12', border: '1px solid #1A1A1F' }}>
        <SectionHeader
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  ...
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: '8px', color: '#3A3A4E', letterSpacing: '0.08em' }}>{label}</span>
                </div>
              ))}
            </div>
          }
        ...
        <div style={{ padding: '12px 16px 8px', height: '180px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={RISK_DISTRIBUTION_24H} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <CartesianGrid strokeDasharray="2 4" stroke="#141418" vertical={false} />
              <XAxis dataKey="hour" tick={{ fill: '#2E2E3E', fontFamily: 'JetBrains Mono', fontSize: 9 }} axisLine={false} tickLine={false} interval={3} />
              <YAxis tick={{ fill: '#2E2E3E', fontFamily: 'JetBrains Mono', fontSize: 9 }} axisLine={false} tickLine={false} />
              <ReferenceLine y={60} stroke="#2A2A3A" strokeDasharray="3 4" label={{ value: 'threshold', position: 'insideTopRight', fill: '#2A2A3A', fontFamily: 'JetBrains Mono', fontSize: 8 }} />
